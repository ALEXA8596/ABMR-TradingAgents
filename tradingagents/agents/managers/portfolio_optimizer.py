            ...
            if target_weight is not None and price > 0:
                ...
                if delta_shares > 0:
                    ...
                    if buy_qty > 0:
                        ...
                        msg = toolkit.buy.invoke({"company_name": company_name, "trade_date": trade_date, "buy_qty": buy_qty})
                    ...
                    if sell_qty > 0:
                        ...
                        msg = toolkit.sell.invoke({"company_name": company_name, "trade_date": trade_date, "sell_qty": sell_qty})
                        action, quantity = "SELL", sell_qty
                        execution_summary.update({"message": msg})
                        print(f"🔄 EXECUTING SELL: {sell_qty} shares of {company_name} at ${price:.2f}")
                else:
                    # Already at target
                    msg = toolkit.hold.invoke({"company_name": company_name, "trade_date": trade_date, "note": "At target weight"})
                ...
                if "BUY" in action_text and price > 0 and liquid > 0:
                    ...
                    msg = toolkit.hold.invoke({"company_name": company_name, "trade_date": trade_date, "note": "No target weight; HOLD"})
                    action, quantity = "HOLD", 0
                    execution_summary.update({"message": msg})
